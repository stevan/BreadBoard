...
Then inside your application you can simply create an
instance of the container and instantiate an instance
of the application.
...
Extending containers is just one way of re-use, just like
...
This container fufills the bare minimum, but this could have
...
C<create> method, like so.
  ...
  my $log_handle = $db_logger->create(
...
And of course, since the Logger is a parameter we could just
as easily pass in a simpler logger in a test environment for
example. Here is what that would look like.
